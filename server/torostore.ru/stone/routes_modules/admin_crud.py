...
def admin_api_archive():
    ...
    avito_id = _safe_str(data.get('avito_id', ''), 64)
    if not avito_id:
        return jsonify({'error': 'avito_id required'}), 400
    ...
    if 'supply_batch_id' in data and data['supply_batch_id']:
        product.supply_batch_id = _safe_int(data['supply_batch_id'])
    if 'sold_price' in data and data['sold_price']:
        product.sold_price = _safe_int(data['sold_price'], min_val=0)
...
def admin_api_add():
    ...
    try:
        title = _safe_str(data.get('title', ''), 255)
        if not title:
            return jsonify({'error': 'title required'}), 400
        price = _safe_int(data.get('price', 0), min_val=0, max_val=9999999)
        ...
        product = StoneProduct(
            avito_id=avito_id,
            title=title,
            description=_safe_str(data.get('description', ''), 5000),
            price=price,
            price_string=f"{price:,} ₽".replace(',', '.'),
            size=_safe_str(data.get('size', ''), 32),
            season=_safe_str(data.get('season', ''), 128),
            art=_safe_str(data.get('art', ''), 64),
            condition=_safe_str(data.get('condition', ''), 32),
            defects=_safe_str(data.get('defects', ''), 512),
            avito_url=_safe_str(data.get('avito_url', ''), 512),
            category=_safe_str(data.get('category', ''), 32),
            images=json.dumps(images_list, ensure_ascii=False),
            defect_images=json.dumps(data.get('defect_images', [])[:10], ensure_ascii=False),
            supply_batch_id=_safe_int(data.get('supply_batch_id')) or None,
            cost_price=_safe_int(data.get('cost_price', 0), min_val=0),
            avito_cost=_safe_int(data.get('avito_cost', 0), min_val=0),
            repair_cost=_safe_int(data.get('repair_cost', 0), min_val=0),
            sold_price=_safe_int(data.get('sold_price', 0), min_val=0),
            old_price=_safe_int(data.get('old_price', 0), min_val=0),
            is_active=True
        )
        discount_hours = _safe_int(data.get('discount_hours', 0), min_val=0, max_val=720)
        if discount_hours > 0:
...
def admin_api_update():
    ...
    try:
        field_limits = {
            'title': 255, 'description': 5000, 'size': 32, 'season': 128,
            'art': 64, 'condition': 32, 'defects': 512, 'avito_url': 512, 'category': 32
        }
        for field, max_len in field_limits.items():
            val = data.get(field)
            if val is not None:
                setattr(product, field, _safe_str(val, max_len))
        if data.get('price') is not None:
            price = _safe_int(data['price'], min_val=0, max_val=9999999)
            product.price = price
            product.price_string = f"{price:,} ₽".replace(',', '.')
        ...
        if data.get('defect_images') is not None:
            ...
            if isinstance(defect_imgs, list) and len(defect_imgs) > 10:
                defect_imgs = defect_imgs[:10]
            product.defect_images = json.dumps(defect_imgs, ensure_ascii=False)
        if 'supply_batch_id' in data:
            product.supply_batch_id = _safe_int(data['supply_batch_id']) or None
        cost_fields = ['cost_price', 'avito_cost', 'repair_cost', 'sold_price', 'old_price']
        for field in cost_fields:
            if field in data:
                val = _safe_int(data[field], min_val=0)
                if field in ['cost_price', 'avito_cost', 'repair_cost']:
                    if val > 0 or getattr(product, field) == 0:
                        setattr(product, field, val)
                else:
                    setattr(product, field, val)
        if 'discount_hours' in data:
            hours = _safe_int(data['discount_hours'], min_val=0, max_val=720)
            if hours > 0:
...
def admin_api_cost_bulk():
    ...
    if len(items) > BULK_LIMIT:
        return jsonify({'error': f'Too many items. Max {BULK_LIMIT}'}), 400
    updated = 0
    ...
    for item in items:
        ...
        if 'cost_price' in item:
            val = _safe_int(item['cost_price'], min_val=0)
            if val > 0 or p.cost_price == 0:
                p.cost_price = val
        if 'avito_cost' in item:
            val = _safe_int(item['avito_cost'], min_val=0)
            if val > 0 or p.avito_cost == 0:
                p.avito_cost = val
        if 'repair_cost' in item:
            val = _safe_int(item['repair_cost'], min_val=0)
