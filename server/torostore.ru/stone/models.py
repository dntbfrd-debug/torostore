...
class StoneChatMessage(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    __table_args__ = (db.Index('idx_chat_avito_created', 'avito_id', 'created_at'),)
    def to_dict(self):
        return {'id': self.id, 'avito_id': self.avito_id, 'user_name': self.user_name, 'user_contact': self.user_contact, 'message': self.message, 'is_owner': self.is_owner, 'created_at': self.created_at.isoformat() + 'Z' if self.created_at else None}
